...
export function sendResetPassword(account: AccountRecord): Promise<any> {
    ...
    const resetUrl = new URL(process.env.BH_CLIENT_HTTPS === 'true' ? 'https://p' : 'http://p');
    resetUrl.hostname = process.env.BH_CLIENT_HOSTNAME;
    resetUrl.port = process.env.KBH_CLIENT_PORT;
...
<section>
  ...
  `;
    ...
    form.append('subject', 'Bughive Password Reset');
